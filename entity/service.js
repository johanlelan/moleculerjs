...
module.exports = {
		...
		remove: {
			async handler(ctx) {
				...
				if (entity) {
					...
					const disabledEntity = this.disable(entity);
					// emit entity.removed event
					this.broker.broadcast("entity.removed", disabledEntity);
     ...
     */
		async get(id) {
			const events = await this.broker.call("eventStore.getAllEvents", { id });
			if (_.isEmpty(events)) {
				return undefined;
			}
