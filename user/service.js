...
module.exports = {
  ...
  /**
	 ...
	 */
  settings: {
    /** mail is active only if a service "mail" is available on the broker */
    mailActive: false,
  ...
  methods: {
    ...
    async sendMail(ctx, user, template, data) {
      if (!this.settings.mailActive) return Promise.resolve();
      try {
  ...
  events: {
    "**": {
      async handler(payload, sender, event) {
        if (_.get(payload, "node.id") === "mail") {
          this.settings.mailActive = event === "$node.connected";
        }
      }
    },
  }
